...
public interface InboundFileSynchronizer {
	...
	/**
	 * The operation synchronizes the remote S3 file system with the local directory It retrieved new files and updates existing ones with the latest content from S3 Please note
	 * that this method will NOT delete any additional files present on the local filesystem
	 ...
	 * @param remoteFolder The folder name in S3 whose contents are to be synchronized use / if the contents of the bucket starting from the root are to be synchronized. This
	 * operation will only synchronize the files resent in the given remote folder and will ignore all the folders and sub folder in it.
	 */
	...
	/**
	 * Sets the simple file name wildcard to match to match the file e.g., it can be set to *.txt to accept all .txt files
	 */
	void setFileWildcard(String wildcardString);

	/**
	 * Set the value to true to enable the objects to be accepted even if they are present in the sub folder of the remote folder passed to {@link
